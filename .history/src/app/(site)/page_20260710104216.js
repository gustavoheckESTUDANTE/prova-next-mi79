...
export default async function HomePage() {
    ...
    const cardsComDestaque = cards.filter((card) => card.destaque == true);
    const cardsSemDestaque = cards.filter((card) => card.destaque == false);
    ...
    return <>
            ...
            <div className="mx-auto max-w-6xl px-4 py-16">
                <div className="max-w-3xl">
                    ...
                    <div className="mt-8 flex flex-wrap gap-3">
                        <Button href={"#cursos"} color={}>Explorar cursos</Button>
                        <Button>Conhecer a WEG</Button>
        ...
        <section className="mx-auto max-w-6xl px-4 py-14" id="cursos">
            ...
            <div className="grid gap-5 md:grid-cols-3">
                {cardsComDestaque.map(card => <UserCard curso={card} key={card.id}></UserCard>)}
                {cardsSemDestaque.map(card => <UserCard curso={card} key={card.id}></UserCard>)}
            </div>
        </section>
    </>
}