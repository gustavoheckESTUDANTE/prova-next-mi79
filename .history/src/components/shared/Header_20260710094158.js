    ...
    return <>
        <header className="border-b border-slate-200 bg-white">
            <div className="mx-auto flex max-w-6xl flex-col gap-4 px-4 py-4 md:flex-row md:items-center md:justify-between">
                <a href="/" onClick={() => {setButtonState(buttonNavStates.COURSES); console.log("alterando estado")}} className="flex items-center gap-3" aria-label="Página inicial WEG Academy">
                ...
                <nav className="flex items-center gap-2 text-sm font-semibold text-slate-600" aria-label="Navegação principal">
                    <a href="/" onClick={() => {setButtonState(buttonNavStates.COURSES); console.log("alterando estado")} className={"rounded-full px-3 py-2 " + cursosButtonClassName}>Cursos</a>
