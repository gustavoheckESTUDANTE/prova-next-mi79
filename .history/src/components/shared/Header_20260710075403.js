...
export default function Header() {
    ...
    if (buttonState == buttonNavStates.COURSES) {
        buttonClassName = "bg-weg-light"
        aboutButtonClassName = 
    }
        ...
        <header className="border-b border-slate-200 bg-white">
            <div className="mx-auto flex max-w-6xl flex-col gap-4 px-4 py-4 md:flex-row md:items-center md:justify-between">
                ...
                <nav className="flex items-center gap-2 text-sm font-semibold text-slate-600" aria-label="Navegação principal">
                    ...
                    <a href="sobre" onClick={() => setButtonState(buttonNavStates.ABOUT)} className="rounded-full px-3 py-2 hover:bg-slate-100">Sobre a WEG</a>
                </nav>
            </div>
        </header>
    </>
