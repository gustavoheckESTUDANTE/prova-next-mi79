...
export default function UserCard({ curso }) {
    ...
    const dataFormatada = new Intl.DateTimeFormat("pt-BR", {timeStyle: "short", dateStyle: "short"}).format(Date.parse(curso.data));

    return <>
        <d className="border-t border-slate-200 bg-white">
            <article className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
                ...
                <div className="p-5">
                    {
                        curso.destaque && <span className="inline-flex rounded-full bg-emerald-50 px-3 py-1 text-xs font-extrabold uppercase tracking-wide text-emerald-700">Destaque</span>
                    }
                    ...
                    <div className="mt-4 flex flex-wrap gap-2 text-xs font-bold text-slate-600">
                        <span className="rounded-full bg-slate-100 px-3 py-1">{precoFormatado}</span>
                        <span className="rounded-full bg-slate-100 px-3 py-1">{dataFormatada}</span>
