    ...
    return <>
        <footer className="border-t border-slate-200 bg-white">
            <article class="overflow-hidden rounded-2xl border border-slate-200 bg-white">
                <Image src={curso.imagem} width={200} height={200} alt={`Imagem placeholder do curso ${curso.nome}`} class="h-44 w-full object-cover"></Image>
                    <div class="p-5">
                        ...
                        <div class="mt-4 flex flex-wrap gap-2 text-xs font-bold text-slate-600">
                            ...
                            <span class={"rounded-full px-3 py-1 " + (curso.vagas_disponiveis > 10 ? "bg-slate-100" : "bg-amber-50 text-amber-700")}>{curso.vagas_disponiveis}</span>
                        </div>
