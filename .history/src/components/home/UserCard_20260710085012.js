...
export default function Footer(curso) {
    return <>
        <footer className="border-t border-slate-200 bg-white">
            <article class="overflow-hidden rounded-2xl border border-slate-200 bg-white">
                <Image src="./assets/img/placeholder-curso.jpg" alt={`Imagem placeholder do curso ${curso.nome}`} class="h-44 w-full object-cover"></Image>
                    <div class="p-5">
                        ...
                        <div class="mt-4 flex flex-wrap gap-2 text-xs font-bold text-slate-600">
                            ...
                            <span class={"rounded-full bg-slate-100 px-3 py-1 " + (curso.vagas_disponiveis > 10 ? "bg-slate-100" : "bg-amber-50")}>{curso.vagas_disponiveis}</span>
                        </div>

                        <p class="mt-4 text-sm text-slate-500">{curso.local}</p>
                        <a href="/id" class="mt-5 inline-flex text-sm font-extrabold text-weg-blue hover:text-weg-dark">Ver detalhes</a>
                    </div>
            </article>
        </footer>
    </>
